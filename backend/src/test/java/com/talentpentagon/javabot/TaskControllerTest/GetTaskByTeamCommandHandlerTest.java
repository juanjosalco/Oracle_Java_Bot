...
public class GetTaskByTeamCommandHandlerTest {
    ...
    public void getTaskByTeamCommandHandler_validFilter_returnsTask() {
        ...
        // Create a list of TaskItem objects for mocking the teamService.getTeamTasks()
        // method
        ...
        // objects
        when(teamService.getTeamTasks(any(Integer.class), any(String.class), any(String.class)))
                .thenReturn(taskItemList);

        // Mock the behavior of teamRepository to return a team when findById is called
        when(teamRepository.findById(1)).thenReturn(Optional.of(team));

        // Call the execute method of the getTaskByTeamHandler and capture the response
        ResponseEntity<List<TaskItem>> response = getTaskByTeamHandler.execute(1, "Priority", "In Progress");
