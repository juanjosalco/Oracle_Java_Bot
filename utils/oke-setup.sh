...
while ! state_done SSL; do
  mkdir -p $TALENT_PENTAGON_LOCATION/tls
  openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout $TALENT_PENTAGON_LOCATION/tls/tls.key -out $TALENT_PENTAGON_LOCATION/tls/tls.crt -subj "/CN=grabdish/O=grabdish"
  state_set_done SSL
done
...
while ! state_done OKE_OCID; do
  OKE_OCID=$(terraform -chdir="${TALENT_PENTAGON_LOCATION}"/terraform output -json | python "$TALENT_PENTAGON_LOCATION"/utils/python-scripts/process-cluster-ocid-json.py)
...
while ! state_done OKE_NAMESPACE; do
  if kubectl create ns talent-pentagon; then
...
while ! state_done APP_DATA; do
  echo "`date`: Waiting for APP_DATA"
  sleep 2
done
